...
<div class="flex min-h-screen flex-col">
	...
	<div class="fixed inset-0 -z-10 overflow-hidden">
		<div
			class="absolute -right-1/4 -top-1/2 h-[800px] w-[800px] rounded-full bg-gradient-to-br from-amber-500/10 to-orange-500/10 blur-3xl"
		></div>
		<div
			class="absolute -bottom-1/2 -left-1/4 h-[600px] w-[600px] rounded-full bg-gradient-to-tr from-orange-500/10 to-amber-500/10 blur-3xl"
		></div>
	</div>
	...
	<main class="flex-1 px-4 pb-12 pt-28 sm:px-6 lg:px-8">
		<div class="mx-auto max-w-5xl">
			...
			<div class="mb-8 text-center" in:fade={{ duration: 200 }}>
				<div
					class="mb-4 inline-flex items-center gap-2 rounded-full bg-amber-500/10 px-4 py-1.5 text-sm font-medium text-amber-400"
				>
					<Crop class="h-4 w-4" />
					Crop GIF
				</div>
				...
				<p class="text-surface-500 mt-2">Perfect for Discord emojis, memes, removing borders</p>
			</div>
				...
				<div class="grid gap-6 lg:grid-cols-2" in:fly={{ y: 20, duration: 300 }}>
					...
					<div class="glass rounded-2xl p-4">
						<div
							...
							class="bg-surface-900 relative select-none overflow-hidden rounded-xl"
						...
						>
							...
							<!-- Crop Selection Box -->
							<div
								class="absolute border-2 border-amber-400 {isDragging
									? 'cursor-grabbing'
									: 'cursor-grab'}"
								style="
									left: {(cropX / gifWidth) * 100}%;
							...
							>
								<!-- Grid overlay -->
								<div
									class="pointer-events-none absolute inset-0 grid grid-cols-3 grid-rows-3 opacity-40"
								>
									{#each Array(9) as _, i}
								...
								<div
									class="absolute -left-2.5 -top-2.5 h-5 w-5 cursor-nw-resize touch-none rounded-sm bg-amber-400 sm:-left-1.5 sm:-top-1.5 sm:h-3 sm:w-3"
									onmousedown={(e) => {
										e.stopPropagation();
								...
								></div>
								<div
									class="absolute -right-2.5 -top-2.5 h-5 w-5 cursor-ne-resize touch-none rounded-sm bg-amber-400 sm:-right-1.5 sm:-top-1.5 sm:h-3 sm:w-3"
									onmousedown={(e) => {
										e.stopPropagation();
								...
								></div>
								<div
									class="absolute -bottom-2.5 -left-2.5 h-5 w-5 cursor-sw-resize touch-none rounded-sm bg-amber-400 sm:-bottom-1.5 sm:-left-1.5 sm:h-3 sm:w-3"
									onmousedown={(e) => {
										e.stopPropagation();
								...
								></div>
								<div
									class="absolute -bottom-2.5 -right-2.5 h-5 w-5 cursor-se-resize touch-none rounded-sm bg-amber-400 sm:-bottom-1.5 sm:-right-1.5 sm:h-3 sm:w-3"
									onmousedown={(e) => {
										e.stopPropagation();
								...
								<div
									class="absolute -top-2.5 left-1/2 h-5 w-8 -translate-x-1/2 cursor-n-resize touch-none rounded-sm bg-amber-400 sm:-top-1.5 sm:h-3 sm:w-6"
									onmousedown={(e) => {
										e.stopPropagation();
								...
								></div>
								<div
									class="absolute -bottom-2.5 left-1/2 h-5 w-8 -translate-x-1/2 cursor-s-resize touch-none rounded-sm bg-amber-400 sm:-bottom-1.5 sm:h-3 sm:w-6"
									onmousedown={(e) => {
										e.stopPropagation();
								...
								></div>
								<div
									class="absolute -left-2.5 top-1/2 h-8 w-5 -translate-y-1/2 cursor-w-resize touch-none rounded-sm bg-amber-400 sm:-left-1.5 sm:h-6 sm:w-3"
									onmousedown={(e) => {
										e.stopPropagation();
								...
								></div>
								<div
									class="absolute -right-2.5 top-1/2 h-8 w-5 -translate-y-1/2 cursor-e-resize touch-none rounded-sm bg-amber-400 sm:-right-1.5 sm:h-6 sm:w-3"
									onmousedown={(e) => {
										e.stopPropagation();
						...
						<div class="mt-4 flex items-center justify-between">
							<div class="text-sm">
								<p class="text-surface-300 max-w-[200px] truncate font-medium">{gifFile.name}</p>
								{#if metadata}
							...
							<button
								onclick={clearGif}
								class="text-surface-400 flex items-center gap-1.5 text-sm transition-colors hover:text-red-400"
							>
					...
					<div class="glass h-fit rounded-2xl p-6" in:fly={{ y: 20, delay: 100, duration: 200 }}>
								...
								{#each aspectPresets as preset}
									<button
										...
										class="rounded-lg px-3 py-2 text-sm transition-all {selectedAspect === preset.id
											? 'text-surface-100 border border-amber-500/50 bg-gradient-to-r from-amber-500/20 to-orange-500/20'
						...
						<div class="bg-surface-800/50 mb-6 rounded-xl p-4">
							...
							<div class="grid grid-cols-2 gap-4 text-sm">
								<div>
									...
									<span class="ml-2 text-amber-400">{cropX}, {cropY}</span>
								</div>
								<div>
									<span class="text-surface-500">Size:</span>
									<span class="ml-2 text-amber-400">{cropWidth} × {cropHeight}</span>
								</div>
							</div>
						</div>
						...
						<button
							onclick={resetCrop}
							class="bg-surface-700 text-surface-300 hover:bg-surface-600 mb-4 flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium transition-colors"
						>
						...
						<button
							...
							class="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 px-6 py-3 text-base font-semibold text-white shadow-lg shadow-amber-500/30 transition-all hover:scale-[1.02] hover:shadow-xl hover:shadow-amber-500/40 disabled:cursor-not-allowed disabled:opacity-50"
						>
							{#if isProcessing}
						...
						<!-- Progress -->
						{#if isProcessing}
							<div class="bg-surface-800 mt-4 h-2 overflow-hidden rounded-full">
								<div
						...
						{#if resultUrl}
							...
							>
								<div class="mb-3 flex items-center justify-between">
									<div>
										<p class="font-medium text-green-400">Crop complete!</p>
										<p class="text-surface-500 text-sm">
											{cropWidth}×{cropHeight} • {formatBytes(resultSize)} • {(
												processingTime / 1000
											).toFixed(1)}s
										</p>
									</div>
								...
								<!-- Preview -->
								<div
									class="bg-surface-900 mb-3 flex max-h-32 items-center justify-center overflow-hidden rounded-lg"
								>
									<img
										src={resultUrl}
										alt="Cropped result"
