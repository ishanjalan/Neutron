...
<svelte:head>
	...
	<meta
		name="description"
		content="Export every frame of a GIF as individual PNG images. Free and private."
	/>
...
<div class="flex min-h-screen flex-col">
	...
	<div class="fixed inset-0 -z-10 overflow-hidden">
		<div
			class="absolute -top-1/2 -right-1/4 h-[800px] w-[800px] rounded-full bg-gradient-to-br from-emerald-500/10 to-teal-500/10 blur-3xl"
		></div>
		<div
			...
			{:else}
				<div in:fly={{ y: 20, duration: 200 }}>
					...
					<div class="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8">
						{#each frames as frame, i (frame.url)}
							<div class="group relative">
								...
								>
									...
									<!-- Delay -->
									<div
										class="absolute right-1 bottom-1 rounded bg-black/70 px-1.5 py-0.5 text-xs text-white"
									>
										{frame.delay}ms
									</div>
									...
									<!-- Selection indicator -->
									<div
										class="absolute top-2 right-2 flex h-6 w-6 items-center justify-center rounded-full transition-all {frame.selected
											? 'bg-emerald-500'
								...
								<button
									...
									class="bg-surface-800/80 hover:bg-surface-700 absolute top-2 left-2 flex h-6 w-6 items-center justify-center rounded-full opacity-0 transition-opacity group-hover:opacity-100"
