...
>
	...
	<div class="absolute top-4 left-4 z-20 flex items-center gap-4">
		<div class="bg-surface-800/90 rounded-xl px-4 py-2 backdrop-blur-sm">
			<div class="text-surface-400 text-xs tracking-wide uppercase">Original</div>
		...
		<div class="from-accent-start to-accent-end rounded-xl bg-gradient-to-r px-4 py-2 shadow-lg">
			<div class="text-xs tracking-wide text-white/80 uppercase">Compressed</div>
	...
	<!-- Instructions -->
	<div
		class="absolute top-4 left-1/2 hidden -translate-x-1/2 rounded-full bg-white/10 px-4 py-2 text-sm text-white backdrop-blur-sm md:block"
	>
	...
	<div
		...
		class="bg-surface-900 relative aspect-auto w-[95vw] max-w-4xl touch-none overflow-hidden rounded-2xl shadow-2xl select-none"
	...
	>
		...
		>
			<!-- Handle circle -->
			<div
				class="absolute top-1/2 left-1/2 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-white shadow-xl transition-transform {isDragging
					? 'scale-110'
					: ''}"
			>
				<svg
		...
		<div
			class="from-accent-start to-accent-end pointer-events-none absolute right-4 bottom-4 rounded-full bg-gradient-to-r px-3 py-1.5 text-xs font-semibold text-white shadow-lg"
		>
			Compressed
		</div>
	</div>
