<script lang="ts">
	...
	import { preloadEncoder, checkBrowserSupport } from '$lib/utils/compress';
			...
			{#if !hasVideos}
				<div class="mb-8 text-center sm:mb-12" in:fade={{ duration: 300 }}>
					<h1 class="mb-4 text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">
						<span class="gradient-text">Compress videos.</span>
						<span class="text-surface-400"> GPU-accelerated.</span>
					</h1>
					<p class="text-surface-500 mx-auto mb-8 max-w-2xl text-base leading-relaxed sm:text-lg">
						...
						>
						+ WebCodecs. H.264, H.265, VP9, AV1 — all processed locally in your browser.
					</p>
					...
					<!-- Clipboard hint -->
					<div class="text-surface-500 flex items-center justify-center gap-2">
						<Clipboard class="h-4 w-4" strokeWidth={1.5} />
						<span class="text-sm"
