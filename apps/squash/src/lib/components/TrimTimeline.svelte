...
<div class="select-none">
	...
	>
		...
		<div class="absolute top-0 bottom-0 left-0 bg-black/60" style="width: {startPercent}%"></div>
		<div
			class="absolute top-0 right-0 bottom-0 bg-black/60"
		...
		<!-- Start handle -->
		<div
			class="absolute top-0 bottom-0 z-10 flex w-3 cursor-ew-resize items-center justify-center bg-purple-500 transition-all {isDraggingStart
		...
		<!-- End handle -->
		<div
			class="absolute top-0 bottom-0 z-10 flex w-3 cursor-ew-resize items-center justify-center bg-purple-500 transition-all {isDraggingEnd
				? 'ring-2 ring-purple-300'
				: 'hover:bg-purple-400'}"
		...
		{#if previewTime !== null && !isDraggingStart && !isDraggingEnd}
			<div
				class="bg-surface-700 text-surface-200 absolute -top-8 z-20 -translate-x-1/2 transform rounded px-2 py-1 text-xs whitespace-nowrap"
