		...
		{#each videos.items as item, index (item.id)}
			...
			>
				...
				{#if dragOverIndex === index && draggedItem?.id !== item.id}
					<div
						class="bg-accent-start absolute -top-2 right-0 left-0 h-1 rounded-full"
				...
				{#if item.status === 'pending'}
					<div
						class="bg-surface-700 text-surface-300 ring-surface-900 absolute -top-1 -left-1 flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold ring-2"
					>
						{index + 1}
					</div>
				{/if}
