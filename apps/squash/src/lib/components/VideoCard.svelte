...
>
	...
	<button
		...
		class="bg-surface-700 text-surface-400 absolute -top-2 -right-2 z-10 flex h-6 w-6 items-center justify-center rounded-full opacity-0 shadow-lg transition-all group-hover:opacity-100 hover:bg-red-500 hover:text-white"
	...
	<div class="bg-surface-800 relative aspect-video w-full overflow-hidden rounded-t-2xl">
		...
		{#if item.width && item.height}
			<div
				class="absolute right-2 bottom-2 rounded bg-black/70 px-1.5 py-0.5 text-xs font-medium text-white"
			>
		...
		{#if item.status === 'completed' && item.compressedBlob}
			<div
				class="absolute top-2 left-2 flex items-center gap-1 rounded bg-black/70 px-1.5 py-0.5 text-xs font-medium text-white opacity-0 transition-opacity group-hover:opacity-100"
			>
				...
				<div class="flex items-center justify-between">
					...
					<div class="relative">
						<button
							...
							class="flex items-center gap-1 rounded bg-gradient-to-r {getCurrentFormatColor()} px-2 py-0.5 text-xs font-bold text-white uppercase transition-all hover:opacity-90"
						>
