	...
	<div class="p-4 sm:p-6">
		<div class="flex flex-wrap items-center gap-3 sm:gap-4">
			...
			<div class="flex flex-col gap-1.5">
				<span class="text-surface-500 hidden text-[10px] tracking-wider uppercase sm:block"
					>Quality</span
				>
			...
			<div class="flex flex-col gap-1.5">
				...
				<div class="relative">
					...
					{#if showSizePresets}
						...
						></button>
						<div
							class="bg-surface-800 absolute top-full left-0 z-50 mt-2 w-48 overflow-hidden rounded-xl shadow-xl ring-1 ring-white/10"
						...
						>
							...
							<div class="border-surface-700/50 border-b p-3">
								<div class="flex items-center gap-2">
									<input
										type="text"
										class="bg-surface-700 border-surface-600 text-surface-200 placeholder-surface-500 focus:ring-accent-start focus:border-accent-start flex-1 rounded-lg border px-3 py-1.5 text-sm focus:ring-1 focus:outline-none"
							...
							<!-- Presets -->
							<div class="p-1.5">
								{#each sizePresets as preset (preset.key)}
									<button
				...
				{#each formats as format (format.value)}
					...
					>
						...
						{#if isHardwareCodec && isAvailable}
							<span
								class="absolute -top-1.5 -right-1.5 rounded bg-purple-500 px-1 py-0.5 text-[9px] font-bold text-white"
							>
								GPU
							</span>
		...
		>
			<div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
				...
				<div>
					...
					<div class="grid grid-cols-2 gap-1.5">
						{#each audioCodecs as codec (codec.value)}
							<button
					...
					{#if videos.settings.audioCodec !== 'none' && videos.settings.audioCodec !== 'copy'}
						<div class="mt-3">
							...
							<div class="flex gap-1">
								{#each AUDIO_BITRATE_OPTIONS as option (option.value)}
									<button
				...
				<div>
					...
					<div class="space-y-1.5">
						{#each encodingPresets as preset (preset.value)}
							<button
