	...
	>
		...
		>
			...
			>
				...
				<div
					class="bg-surface-800 ring-surface-900 absolute -right-1 -bottom-1 flex h-8 w-8 items-center justify-center rounded-full ring-2"
				>
			...
			{#if !hasItems}
				<div class="flex flex-wrap justify-center gap-2">
					{#each formats as format (format.name)}
						<span
							class="rounded-full bg-gradient-to-r {format.color} px-3 py-1 text-xs font-bold text-white uppercase shadow-lg"
						>
							{format.name}
						</span>
					{/each}
