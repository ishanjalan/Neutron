<script lang="ts">
	...
	function handleDownload() {
		...
		downloadBlob(currentFile, getOutputFilename(currentFile.name, 'delete-pages'));
	}
...
<div class="flex flex-col" style="height: 100dvh;">
	...
	{#if currentFile}
		...
		>
			<Trash2 class="h-4 w-4 flex-shrink-0 text-red-400" />
			...
			<button
				...
				disabled={!hasChanges}
				class="flex flex-shrink-0 items-center gap-1.5 rounded-lg bg-red-500 px-3 py-1.5 text-xs font-semibold text-white transition-all hover:bg-red-600 disabled:cursor-not-allowed disabled:opacity-40"
			>
				...
				<div class="flex flex-col items-center justify-center gap-4 px-6 text-center">
					...
					<div>
						...
						<p class="text-surface-500 mt-1 text-sm">
							Hover over any thumbnail and click the delete button to remove that page
						</p>
					</div>
					<div
						class="mt-2 inline-flex items-center gap-2 rounded-full bg-red-500/10 px-4 py-1.5 text-sm font-medium text-red-400"
					>
