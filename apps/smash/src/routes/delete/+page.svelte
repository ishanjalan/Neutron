<script lang="ts">
	...
	async function handleDelete() {
		...
		try {
			const result = await deletePages(pdfFile.file, {
				pages,
				onProgress: (p) => {
					progress = p;
				},
...
<div class="flex min-h-screen flex-col">
	...
	<div class="fixed inset-0 -z-10 overflow-hidden">
		<div
			class="absolute -right-1/4 -top-1/2 h-[800px] w-[800px] rounded-full bg-gradient-to-br from-red-500/10 to-pink-500/10 blur-3xl"
		></div>
	</div>
	...
	<main class="flex-1 px-4 pb-12 pt-28 sm:px-6 lg:px-8">
		<div class="mx-auto max-w-4xl">
			<div class="mb-8 text-center" in:fade={{ duration: 200 }}>
				...
				<h1 class="text-surface-100 text-3xl font-bold">Remove pages from your PDF</h1>
				<p class="text-surface-500 mt-2">Select pages to delete permanently</p>
			</div>
			...
			<div class="grid gap-6 lg:grid-cols-2">
					...
					{#if !hasFile}
						...
						>
							...
							<div class="flex flex-col items-center justify-center gap-4 px-6">
								<div
									class="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-red-500/20 to-pink-500/20"
								>
						...
						<div class="glass rounded-2xl p-4" in:fly={{ y: 20, duration: 200 }}>
							...
							{#if isProcessing}<div class="mt-4">
									<div class="bg-surface-700 h-2 overflow-hidden rounded-full">
										<div
											class="h-full bg-gradient-to-r from-red-500 to-pink-500 transition-all"
							...
							>
								<div class="flex items-center justify-between">
									<p class="flex items-center gap-2 font-medium text-green-400">
										<CheckCircle class="h-4 w-4" /> Pages deleted!
									</p>
									<button
				...
				<div class="glass rounded-2xl p-6" in:fly={{ y: 20, delay: 100, duration: 200 }}>
					<h3 class="text-surface-100 mb-6 flex items-center gap-2 text-lg font-semibold">
						<Settings class="text-accent-start h-5 w-5" /> Delete Options
					</h3>
					<div>
						<label class="text-surface-300 mb-2 block text-sm font-medium">Pages to Delete</label>
						<input
							type="text"
						...
						<p class="text-surface-500 mt-1 text-xs">
							{#if pdfFile}Document has {pdfFile.pageCount} pages{:else}Enter page numbers or ranges{/if}
						</p>
					</div>
					<button
						onclick={handleDelete}
						disabled={!hasFile || isProcessing || !pageRange}
						class="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-red-500 to-pink-500 px-6 py-3 text-base font-semibold text-white shadow-lg transition-all hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:scale-100"
					>
						{#if isProcessing}<Loader2 class="h-5 w-5 animate-spin" /> Deleting...{:else}<Trash2
								class="h-5 w-5"
							/> Delete Pages{/if}
					</button>
				</div>
