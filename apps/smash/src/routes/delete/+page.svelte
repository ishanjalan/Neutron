<script lang="ts">
	...
	import { Trash2, Upload, FileText, Download, Loader2, CheckCircle, Settings } from 'lucide-svelte';
	...
	async function handleDelete() {
		...
		if (pages.length === 0) { toast.error('No valid pages selected'); return; }

		isProcessing = true;
		progress = 0;

		try {
			const result = await deletePages(pdfFile.file, { pages, onProgress: (p) => { progress = p; } });
			resultBlob = result;
			toast.success(`Deleted ${pages.length} page(s)`);
		} catch (error) {
	...
	function downloadResult() {
		...
		const filename = getOutputFilename(pdfFile.file.name, 'delete-pages');
...
<div class="flex min-h-screen flex-col">
	...
	<div class="fixed inset-0 -z-10 overflow-hidden">
		<div class="absolute -top-1/2 -right-1/4 h-[800px] w-[800px] rounded-full bg-gradient-to-br from-red-500/10 to-pink-500/10 blur-3xl"></div>
	</div>

	<main class="flex-1 px-4 sm:px-6 lg:px-8 pt-28 pb-12">
		<div class="mx-auto max-w-4xl">
			<div class="text-center mb-8" in:fade={{ duration: 200 }}>
				<div class="inline-flex items-center gap-2 rounded-full bg-red-500/10 px-4 py-1.5 text-sm font-medium text-red-400 mb-4">
					<Trash2 class="h-4 w-4" />
					Delete Pages
				</div>
				<h1 class="text-3xl font-bold text-surface-100">Remove pages from your PDF</h1>
				<p class="mt-2 text-surface-500">Select pages to delete permanently</p>
			</div>
			...
			<div class="grid gap-6 lg:grid-cols-2">
				<div>
					{#if !hasFile}
						<div role="button" tabindex="0" ondragenter={handleDragEnter} ondragleave={handleDragLeave} ondragover={handleDragOver} ondrop={handleDrop} onclick={openFilePicker} onkeydown={(e) => e.key === 'Enter' && openFilePicker()} class="relative overflow-hidden rounded-2xl border-2 border-dashed transition-all duration-300 cursor-pointer py-16 {isDragging ? 'border-accent-start bg-accent-start/10 scale-[1.02]' : 'border-surface-700 hover:border-surface-600 bg-surface-900/50'}">
							...
							<div class="flex flex-col items-center justify-center gap-4 px-6">
								<div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-red-500/20 to-pink-500/20"><Upload class="h-8 w-8 text-red-400" /></div>
					...
					{:else if pdfFile}
						<div class="glass rounded-2xl p-4" in:fly={{ y: 20, duration: 200 }}>
							...
							{#if isProcessing}<div class="mt-4"><div class="h-2 bg-surface-700 rounded-full overflow-hidden"><div class="h-full bg-gradient-to-r from-red-500 to-pink-500 transition-all" style="width: {progress}%"></div></div></div>{/if}
						</div>
						{#if resultBlob}
							<div class="mt-4 p-4 rounded-xl bg-green-500/10 border border-green-500/30" in:fly={{ y: 10, duration: 200 }}>
								<div class="flex items-center justify-between">
									<p class="text-green-400 font-medium flex items-center gap-2"><CheckCircle class="h-4 w-4" /> Pages deleted!</p>
				...
				<div class="glass rounded-2xl p-6" in:fly={{ y: 20, delay: 100, duration: 200 }}>
					<h3 class="flex items-center gap-2 text-lg font-semibold text-surface-100 mb-6"><Settings class="h-5 w-5 text-accent-start" /> Delete Options</h3>
					<div>
						<label class="block text-sm font-medium text-surface-300 mb-2">Pages to Delete</label>
						<input type="text" bind:value={pageRange} placeholder="e.g., 1-3, 5, 8-10" class="w-full rounded-xl bg-surface-800 border border-surface-700 px-4 py-2.5 text-surface-100 placeholder:text-surface-600 focus:border-accent-start focus:outline-none" />
						<p class="text-xs text-surface-500 mt-1">{#if pdfFile}Document has {pdfFile.pageCount} pages{:else}Enter page numbers or ranges{/if}</p>
					</div>
					<button onclick={handleDelete} disabled={!hasFile || isProcessing || !pageRange} class="mt-6 w-full flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-red-500 to-pink-500 px-6 py-3 text-base font-semibold text-white shadow-lg transition-all hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100">
						{#if isProcessing}<Loader2 class="h-5 w-5 animate-spin" /> Deleting...{:else}<Trash2 class="h-5 w-5" /> Delete Pages{/if}
					</button>
				</div>
			</div>
		</div>
