<script lang="ts">
	...
	import { Minimize2, Upload, FileText, Download, Trash2, Loader2, CheckCircle, AlertCircle, Settings, ChevronDown, Monitor, BookOpen, Printer, FileCheck, Eye } from 'lucide-svelte';
	...
	const presets = {
		...
		ebook: { label: 'Reading', desc: 'Balanced (~50% reduction)', icon: BookOpen, recommended: true },
		...
		for (const pdfFile of pendingFiles) {
			...
			try {
				const result = await compressPDF(pdfFile.file, {
					...
					onProgress: (progress) => {
						const stage = progress < 30 ? 'Initializing...' : progress < 80 ? 'Compressing...' : 'Finalizing...';
		...
		<div class="mx-auto max-w-5xl">
			...
			<div class="grid gap-6 lg:grid-cols-2">
				...
				<div>
					...
					>
						...
						<div class="flex flex-col items-center justify-center gap-4 px-6">
							<div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500/20 to-purple-500/20">
								{#if isDragging}
									<FileText class="h-8 w-8 text-accent-start animate-pulse" />
								{:else}
							...
							<div class="text-center">
								...
								{#if !hasFiles}
									<p class="mt-1 text-sm text-surface-500">Supports PDF files</p>
								{/if}
							</div>
						</div>
					</div>
					...
					{#if hasFiles}
						<div class="mt-4 space-y-2" in:fly={{ y: 20, duration: 200 }}>
							{#each files as pdfFile (pdfFile.id)}
								<div class="glass rounded-xl p-3 flex items-center justify-between" in:slide={{ duration: 200 }}>
											...
											{#if pdfFile.status === 'processing'}
												<div class="mt-1">
													<div class="h-1 bg-surface-700 rounded-full overflow-hidden">
														<div class="h-full bg-gradient-to-r from-accent-start to-accent-end transition-all" style="width: {pdfFile.progress}%"></div>
													</div>
													<p class="text-xs text-surface-500 mt-0.5">{pdfFile.progressStage}</p>
												</div>
											{/if}
											{#if pdfFile.status === 'error'}
												<p class="text-xs text-red-400 mt-0.5">{pdfFile.error}</p>
											{/if}
										</div>
									</div>
				...
				<div class="glass rounded-2xl p-6" in:fly={{ y: 20, delay: 100, duration: 200 }}>
					...
					<!-- Presets -->
					<div class="space-y-3">
						<label class="block text-sm font-medium text-surface-300">Quality Preset</label>
						<div class="grid grid-cols-2 gap-2">
							{#each Object.entries(presets) as [key, preset]}
								{@const Icon = preset.icon}
								<button
									onclick={() => compressionPreset = key as CompressionPreset}
									class="relative px-3 py-3 rounded-xl text-left transition-all {compressionPreset === key
					...
					<!-- Info Box -->
					<div class="mt-6 p-4 rounded-xl bg-surface-800/50 border border-surface-700/50">
						<p class="text-sm text-surface-400">
							<strong class="text-surface-300">How it works:</strong> Your PDF is compressed using Ghostscript WASM directly in your browser. 
							Text remains searchable and selectable. Images are optimized based on your selected quality preset.
						</p>
					</div>
					...
					>
						...
						{:else}
							...
							Compress {pendingCount > 0 ? `${pendingCount} PDF${pendingCount !== 1 ? 's' : ''}` : 'PDFs'}
						{/if}
					</button>
				</div>
			</div>
