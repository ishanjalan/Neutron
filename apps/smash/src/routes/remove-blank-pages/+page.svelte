<script lang="ts">
	...
	import { FileX, Upload, FileText, Download, Trash2, Loader2, CheckCircle } from 'lucide-svelte';
	...
	async function handleProcess() {
		...
		try {
			const originalPageCount = pdfFile.pageCount ?? 0;
			const result = await removeBlankPages(pdfFile.file, {
				threshold,
				onProgress: (p) => {
					progress = p;
				},
			...
			// Estimate removed pages from size difference (rough heuristic)
			// We'll just show a success message
			resultBlob = result;
			resultMessage = originalPageCount > 0 ? `Processed ${originalPageCount} pages` : 'Done';
			toast.success('Blank pages removed!');
		} catch (error) {
			console.error('Remove blank pages error:', error);
			toast.error(error instanceof Error ? error.message : 'Failed to remove blank pages');
		} finally {
			isProcessing = false;
		}
	}
	...
	function downloadResult() {
		...
		const filename = getOutputFilename(pdfFile.file.name, 'cleaned');
	...
	const thresholdLabel = $derived(
		threshold === 0
			? 'Strict — only completely empty pages'
			: threshold <= 10
				? 'Low — nearly empty pages'
				: threshold <= 30
					? 'Medium — pages with minimal content'
					: 'Aggressive — pages with sparse content'
	);
</script>
...
<svelte:head>
	...
	<meta
		...
		content="Automatically detect and remove blank pages from your PDF. Free, private, no upload."
	/>
	<link rel="canonical" href="https://ishanjalan.github.io/Smash/remove-blank-pages" />
</svelte:head>
...
<div class="flex min-h-screen flex-col">
	...
	<div class="fixed inset-0 -z-10 overflow-hidden">
		<div
			class="absolute -top-1/2 -right-1/4 h-[800px] w-[800px] rounded-full bg-gradient-to-br from-red-500/10 to-orange-500/10 blur-3xl"
		></div>
	</div>
	...
	<main class="flex-1 px-4 pt-28 pb-12 sm:px-6 lg:px-8">
		<div class="mx-auto max-w-4xl">
			<div class="mb-8 text-center" in:fade={{ duration: 200 }}>
				<div
					class="mb-4 inline-flex items-center gap-2 rounded-full bg-red-500/10 px-4 py-1.5 text-sm font-medium text-red-400"
				>
					<FileX class="h-4 w-4" /> Remove Blank Pages
				</div>
				<h1 class="text-surface-100 text-3xl font-bold">Remove blank pages from your PDF</h1>
				<p class="text-surface-500 mt-2">Automatically detect and delete empty pages</p>
			</div>
			...
			<div class="grid gap-6 lg:grid-cols-2">
					...
					{#if !hasFile}
						...
						>
							...
							<div class="flex flex-col items-center justify-center gap-4 px-6">
								<div
									class="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-red-500/20 to-orange-500/20"
								>
									<Upload class="h-8 w-8 text-red-400" />
								</div>
						...
						<div class="glass rounded-2xl p-4" in:fly={{ y: 20, duration: 200 }}>
							...
							{#if isProcessing}
								<div class="mt-4">
									...
									<p class="text-surface-500 mt-1 text-xs">Scanning pages for blank content…</p>
								</div>
							{/if}
						</div>
						...
						{#if resultBlob}
							...
							>
								<div class="flex items-center justify-between">
									<div>
										<p class="flex items-center gap-2 font-medium text-red-400">
											<CheckCircle class="h-4 w-4" /> Blank pages removed!
										</p>
										<p class="text-surface-500 text-xs">{resultMessage}</p>
									</div>
									<button
										onclick={downloadResult}
										class="flex items-center gap-2 rounded-lg bg-red-500 px-3 py-1.5 text-sm font-medium text-white hover:bg-red-600"
				...
				<div class="glass rounded-2xl p-6" in:fly={{ y: 20, delay: 100, duration: 200 }}>
					...
					<div class="space-y-5">
						<div>
							...
							<div class="text-surface-600 mt-1 flex justify-between text-xs">
								<span>Strict</span><span>Aggressive</span>
							</div>
						</div>
						...
						<div class="bg-surface-800/50 border-surface-700/50 space-y-1.5 rounded-xl border p-4">
							...
							<p class="text-surface-500 text-xs">
								...
								blank. Higher thresholds also remove pages with very little content.
							</p>
						</div>
					</div>
					...
					<button
						...
						class="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-red-500 to-orange-500 px-6 py-3 text-base font-semibold text-white shadow-lg transition-all hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:scale-100"
					>
						{#if isProcessing}<Loader2 class="h-5 w-5 animate-spin" /> Scanning...{:else}<FileX
								class="h-5 w-5"
							/> Remove Blank Pages{/if}
					</button>
				</div>
			</div>
		</div>
