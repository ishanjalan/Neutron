...
<svelte:head>
	<title>PDF to Images Online — Free | Smash</title>
	<meta
		name="description"
		content="Convert PDF pages to PNG, JPG, or WebP at up to 300 DPI. Free and private."
	/>
	<meta property="og:title" content="PDF to Images Online — Free | Smash" />
	<meta
...
<div class="flex min-h-screen flex-col">
	...
	<div class="fixed inset-0 -z-10 overflow-hidden">
		<div
			class="absolute -top-1/2 -right-1/4 h-[800px] w-[800px] rounded-full bg-gradient-to-br from-teal-500/10 to-green-500/10 blur-3xl"
		></div>
	</div>
	...
	<main class="flex-1 px-4 pt-28 pb-12 sm:px-6 lg:px-8">
		<div class="mx-auto max-w-4xl">
			<div class="mb-8 text-center" in:fade={{ duration: 200 }}>
				...
				<h1 class="text-surface-100 text-3xl font-bold">Convert PDF pages to images</h1>
				<p class="text-surface-500 mt-2">Export each page as PNG, JPG, or WebP</p>
			</div>
			...
			<div class="grid gap-6 lg:grid-cols-2">
					...
					{#if !hasFile}
						...
						>
							...
							<div class="flex flex-col items-center justify-center gap-4 px-6">
								<div
									class="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-teal-500/20 to-green-500/20"
								>
						...
						<div class="glass rounded-2xl p-4" in:fly={{ y: 20, duration: 200 }}>
							...
							{#if isProcessing}<div class="mt-4">
									<div class="bg-surface-700 h-2 overflow-hidden rounded-full">
										<div
											class="h-full bg-gradient-to-r from-teal-500 to-green-500 transition-all"
											style="width: {progress}%"
										></div>
									</div>
									<p class="text-surface-400 mt-1 text-center text-sm">
										Converting page {Math.ceil((progress / 100) * (pdfFile.pageCount || 1))} of {pdfFile.pageCount}...
									</p>
								</div>{/if}
						</div>
							...
							>
								<div class="mb-3 flex items-center justify-between">
									<p class="flex items-center gap-2 font-medium text-green-400">
										...
										{resultBlobs.length} images created
									</p>
									<button
										onclick={downloadAll}
										class="flex items-center gap-2 rounded-lg bg-green-500 px-3 py-1.5 text-sm font-medium text-white hover:bg-green-600"
										><Download class="h-3 w-3" /> Download All</button
									>
								</div>
								<div class="grid max-h-40 grid-cols-4 gap-2 overflow-y-auto">
									{#each resultBlobs as blob, i (i)}
										<button
										...
										>
											...
											<div
												class="absolute bottom-0.5 left-0.5 rounded bg-black/70 px-1 text-[10px] text-white"
											>
												{i + 1}
											</div>
										</button>
				...
				<div class="glass rounded-2xl p-6" in:fly={{ y: 20, delay: 100, duration: 200 }}>
					<h3 class="text-surface-100 mb-6 flex items-center gap-2 text-lg font-semibold">
						<Image class="text-accent-start h-5 w-5" /> Export Settings
					</h3>
					<div class="space-y-4">
						<div>
							...
							<div class="grid grid-cols-3 gap-2">
								{#each ['png', 'jpg', 'webp'] as fmt (fmt)}
									<button
										onclick={() => (imageFormat = fmt as ImageFormat)}
										class="rounded-xl px-3 py-2 text-center text-sm font-bold uppercase transition-all {imageFormat ===
										fmt
						...
						<div>
							...
							<div class="grid grid-cols-3 gap-2">
								{#each [72, 150, 300] as dpi (dpi)}
									<button
										onclick={() => (imageDPI = dpi)}
										class="rounded-xl px-3 py-2 text-center text-sm transition-all {imageDPI === dpi
											? 'bg-accent-start text-white'
											: 'bg-surface-800 text-surface-400 hover:text-surface-200 hover:bg-surface-700'}"
										><span class="font-bold">{dpi}</span> DPI</button
									>
								{/each}
							</div>
						</div>
					...
					<button
						...
						class="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-teal-500 to-green-500 px-6 py-3 text-base font-semibold text-white shadow-lg transition-all hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:scale-100"
					>
						{#if isProcessing}<Loader2 class="h-5 w-5 animate-spin" /> Converting...{:else}<Image
								class="h-5 w-5"
