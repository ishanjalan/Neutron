<script lang="ts">
	...
	const canProcess = $derived(hasFile && watermarkText.trim().length > 0);
	...
	function downloadResult() {
		...
		const filename = getOutputFilename(pdfFile.file.name, 'watermarked');
	...
	<main class="flex-1 px-4 pt-28 pb-12 sm:px-6 lg:px-8">
		<div class="mx-auto max-w-4xl">
			<div class="mb-8 text-center" in:fade={{ duration: 200 }}>
				...
				>
					<Stamp class="h-4 w-4" /> Watermark
				</div>
				<h1 class="text-surface-100 text-3xl font-bold">Add a watermark to your PDF</h1>
				<p class="text-surface-500 mt-2">Diagonal text stamped on every page</p>
			</div>
			...
			<div class="grid gap-6 lg:grid-cols-2">
						...
						{#if resultBlob}
							...
							>
								<div class="flex items-center justify-between">
									<div>
										<p class="flex items-center gap-2 font-medium text-amber-400">
											<CheckCircle class="h-4 w-4" /> Watermark applied!
										</p>
										<p class="text-surface-500 text-xs">"{watermarkText}" on all pages</p>
									</div>
									<button
										onclick={downloadResult}
										class="flex items-center gap-2 rounded-lg bg-amber-500 px-3 py-1.5 text-sm font-medium text-white hover:bg-amber-600"
				...
				<div class="glass rounded-2xl p-6" in:fly={{ y: 20, delay: 100, duration: 200 }}>
					...
					>
						{#if isProcessing}<Loader2 class="h-5 w-5 animate-spin" /> Applying...{:else}<Stamp
								class="h-5 w-5"
							/> Add Watermark{/if}
					</button>
				</div>
