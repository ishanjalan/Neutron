...
<div class="flex min-h-screen flex-col">
	...
	<div class="fixed inset-0 -z-10 overflow-hidden">
		<div
			class="absolute -right-1/4 -top-1/2 h-[800px] w-[800px] rounded-full bg-gradient-to-br from-indigo-500/10 to-violet-500/10 blur-3xl"
		></div>
	</div>
	...
	<main class="flex-1 px-4 pb-12 pt-28 sm:px-6 lg:px-8">
		<div class="mx-auto max-w-4xl">
			<div class="mb-8 text-center" in:fade={{ duration: 200 }}>
				<div
					class="mb-4 inline-flex items-center gap-2 rounded-full bg-indigo-500/10 px-4 py-1.5 text-sm font-medium text-indigo-400"
				>
					<FileText class="h-4 w-4" /> Images to PDF
				</div>
				<h1 class="text-surface-100 text-3xl font-bold">Create a PDF from images</h1>
				<p class="text-surface-500 mt-2">Drag to reorder, then convert to a single PDF</p>
			</div>
			...
			<div
				...
				class="relative cursor-pointer overflow-hidden rounded-2xl border-2 border-dashed transition-all duration-300 {isDragging
					...
					: 'border-surface-700 hover:border-surface-600 bg-surface-900/50'} {hasImages
					? 'py-6'
					: 'py-12'}"
			>
				<input
				...
				<div class="flex flex-col items-center justify-center gap-4 px-6">
					<div
						class="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500/20 to-violet-500/20"
					>
						<Upload class="h-8 w-8 text-indigo-400" />
					</div>
					<p class="text-surface-200 text-lg font-medium">
						{hasImages ? 'Add more images' : 'Drop images here or click to browse'}
					</p>
					{#if !hasImages}<p class="text-surface-500 text-sm">Supports JPG, PNG, WebP</p>{/if}
				</div>
			</div>
			...
			{#if hasImages}
				<div class="mt-6">
					<p class="text-surface-400 mb-3 text-sm">
						Drag to reorder • {images.length} images • {formatBytes(totalSize)}
					</p>
					<div class="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
						{#each images.sort((a, b) => a.order - b.order) as img (img.id)}
							...
							>
								<img src={img.url} alt="" class="bg-surface-800 aspect-[3/4] w-full object-cover" />
								<div
								...
								<button
									...
									class="absolute right-1 top-1 rounded bg-red-500/80 p-1 text-white opacity-0 transition-opacity group-hover:opacity-100"
						...
						<div class="flex items-center justify-between">
							<div>
								<p class="flex items-center gap-2 font-medium text-green-400">
									<CheckCircle class="h-4 w-4" /> PDF created!
								</p>
								<p class="text-surface-500 text-sm">
									{formatBytes(resultBlob.size)} • {images.length} pages
								</p>
							</div>
							<button
				...
				<button
					...
					class="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-500 px-6 py-3 text-base font-semibold text-white shadow-lg transition-all hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:scale-100"
				>
					{#if isProcessing}<Loader2 class="h-5 w-5 animate-spin" /> Creating PDF...{:else}<FileText
							class="h-5 w-5"
						/> Create PDF from {images.length} Images{/if}
				</button>
			{/if}
		</div>
	</main>
