<script lang="ts">
	...
	import { addPageNumbers, getOutputFilename, generateThumbnail, getPageCount } from '$lib/utils/pdf';
	import { formatBytes } from '$lib/stores/pdfs.svelte';
	import { Hash, Upload, FileText, Download, Trash2, Loader2, CheckCircle, Settings } from 'lucide-svelte';
...
<div class="flex min-h-screen flex-col">
	...
	<div class="fixed inset-0 -z-10 overflow-hidden">
		<div class="absolute -top-1/2 -right-1/4 h-[800px] w-[800px] rounded-full bg-gradient-to-br from-slate-500/10 to-gray-500/10 blur-3xl"></div>
	</div>

	<main class="flex-1 px-4 sm:px-6 lg:px-8 pt-28 pb-12">
		<div class="mx-auto max-w-4xl">
			<div class="text-center mb-8" in:fade={{ duration: 200 }}>
				<div class="inline-flex items-center gap-2 rounded-full bg-slate-500/10 px-4 py-1.5 text-sm font-medium text-slate-400 mb-4">
					<Hash class="h-4 w-4" /> Add Page Numbers
				</div>
				<h1 class="text-3xl font-bold text-surface-100">Add page numbers to your PDF</h1>
				<p class="mt-2 text-surface-500">Choose position and starting number</p>
			</div>
			...
			<div class="grid gap-6 lg:grid-cols-2">
				<div>
					{#if !hasFile}
						<div role="button" tabindex="0" ondragenter={handleDragEnter} ondragleave={handleDragLeave} ondragover={handleDragOver} ondrop={handleDrop} onclick={openFilePicker} onkeydown={(e) => e.key === 'Enter' && openFilePicker()} class="relative overflow-hidden rounded-2xl border-2 border-dashed transition-all duration-300 cursor-pointer py-16 {isDragging ? 'border-accent-start bg-accent-start/10 scale-[1.02]' : 'border-surface-700 hover:border-surface-600 bg-surface-900/50'}">
							...
							<div class="flex flex-col items-center justify-center gap-4 px-6">
								<div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-500/20 to-gray-500/20"><Upload class="h-8 w-8 text-slate-400" /></div>
					...
					{:else if pdfFile}
						<div class="glass rounded-2xl p-4" in:fly={{ y: 20, duration: 200 }}>
							...
							{#if isProcessing}<div class="mt-4"><div class="h-2 bg-surface-700 rounded-full overflow-hidden"><div class="h-full bg-gradient-to-r from-slate-500 to-gray-500 transition-all" style="width: {progress}%"></div></div></div>{/if}
						</div>
						{#if resultBlob}
							<div class="mt-4 p-4 rounded-xl bg-green-500/10 border border-green-500/30" in:fly={{ y: 10, duration: 200 }}>
								<div class="flex items-center justify-between">
									<p class="text-green-400 font-medium flex items-center gap-2"><CheckCircle class="h-4 w-4" /> Page numbers added!</p>
				...
				<div class="glass rounded-2xl p-6" in:fly={{ y: 20, delay: 100, duration: 200 }}>
					<h3 class="flex items-center gap-2 text-lg font-semibold text-surface-100 mb-6"><Settings class="h-5 w-5 text-accent-start" /> Page Number Settings</h3>
					<div class="space-y-4">
						<div>
							<label class="block text-sm font-medium text-surface-300 mb-2">Position</label>
							<div class="grid grid-cols-2 gap-2">
								{#each positions as pos}
									<button onclick={() => position = pos.value} class="px-3 py-2 rounded-xl text-sm transition-all {position === pos.value ? 'bg-accent-start text-white' : 'bg-surface-800 text-surface-400 hover:text-surface-200 hover:bg-surface-700'}">{pos.label}</button>
								{/each}
							</div>
						</div>
						<div>
							<label class="block text-sm font-medium text-surface-300 mb-2">Start Number</label>
							<input type="number" min="1" bind:value={startNumber} class="w-full rounded-xl bg-surface-800 border border-surface-700 px-4 py-2.5 text-surface-100 focus:border-accent-start focus:outline-none" />
						</div>
					</div>
					<button onclick={handleAddNumbers} disabled={!hasFile || isProcessing} class="mt-6 w-full flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-slate-500 to-gray-600 px-6 py-3 text-base font-semibold text-white shadow-lg transition-all hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100">
						{#if isProcessing}<Loader2 class="h-5 w-5 animate-spin" /> Adding...{:else}<Hash class="h-5 w-5" /> Add Page Numbers{/if}
					</button>
				</div>
			</div>
		</div>
